...
{
    public static string? AgainstNullsAndEmpties(
    object? arg1,
    bool throws = true,
    [CallerArgumentExpression(Arg1Name)] string arg1Name = null!)
    {
        var nullArgs = GetNullOrEmptyArgsParamNames((arg1Name, arg1));

        return HandleNullAndEmptyArgs(nullArgs, throws);
    }
    ...
    {
        var nullArgs = GetNullOrEmptyArgsParamNames(
            (arg1Name, arg1),
            (arg2Name, arg2));
    ...
    private static List<string> GetNullOrEmptyArgsParamNames(params (string paramName, object? value)[] args)
    {
        var nullArgs = new List<string>();
        foreach (var (paramName, value) in args)
        {
            if (value is null || value is string valueString && string.IsNullOrEmpty(valueString))
    ...
    private static string? HandleNullAndEmptyArgs(List<string> args, bool throws) =>
        HandleArgs(args, "Args cannot be null or empty", throws);
}